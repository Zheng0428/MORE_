...
class LTMemory(nn.Module):
    ...
    def forward(self, key, value, rtg):
        ...
        if len(self.cache_k) < self.context_len:
            new_key = torch.zeros(key.shape[0], key.shape[1], self.hidden_size + (self.context_len * self.compress_size))
            new_value = torch.zeros(value.shape[0], value.shape[1], self.hidden_size + (self.context_len * self.compress_size))
...
class GPT2Attention(nn.Module):
    ...
    def forward(
        ...
        rtg:Optional[torch.FloatTensor] = None                     # add rtg
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        if config.add_cross_attention:
            self.crossattention = GPT2Attention(config, feature_dim = 1, is_cross_attention=True, layer_idx=layer_idx)
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        loss = None
        # Rewrite the part for calculating the loss.
        if labels is not None:
            # Shift so that tokens < n predict n
            lm_labels = torch.argmax(self.lm_head(labels), dim=-1)  # Convert the input labels from one-hot format to class index format.
