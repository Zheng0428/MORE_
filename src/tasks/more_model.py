...
class MOREModel(nn.Module):
    ...
    def forward(self, lxmert_out, rtg, actions, traj_mask, timesteps):
        ...
        output = self.more_decoder(inputs_embeds = lxmert_out, attention_mask = traj_mask, position_ids = timesteps)        #Be sure to pay attention to whether the input sequences are of the same length  #past_key_values = past 后面有时间可以加上
        return output
