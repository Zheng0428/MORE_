...
#        hid_dim = self.more_encoder.dim
        ...
        self.mlp_model = MLPModel(len_i, HIDDEN_NUM, len_o)
    ...
    def forward(self, state, pos, action, reward, target, traj_mask, timesteps):
        """
        ...
        output = self.more_decoder(inputs_embeds = output, labels = target.data['input_ids'].cuda(), attention_mask = traj_mask, postion_ids = timesteps)        #Be sure to pay attention to whether the input sequences are of the same length  #past_key_values = past 后面有时间可以加上
        x = self.more_encoder(action, (state, pos))
        mix_seq = torch.cat((x[0], x[1]), 1)   #mix the output of the lxmert and the reward (numpy)
        output = self.mlp_model(mix_seq)        #
